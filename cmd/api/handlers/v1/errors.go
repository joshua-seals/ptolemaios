...
func (m *Mux) logError(r *http.Request, err error) {
	...
	m.logger.Error(err.Error(), "method", method, "uri", uri)
}
...
func (m *Mux) errorResponse(w http.ResponseWriter, r *http.Request, status int, message any) {
	env := envelope{"error": message}
	// Write the response using the writeJSON() helper. If this happens to return an
	// error then log it, and fall back to sending the client an empty response with a
	// 500 Internal Server Error status code.
	err := m.writeJSON(w, status, env, nil)
	if err != nil {
		m.logError(r, err)
		w.WriteHeader(500)
	}
...
func (m *Mux) serverErrorResponse(w http.ResponseWriter, r *http.Request, err error) {
	m.logError(r, err)
...
func (m *Mux) notFoundResponse(w http.ResponseWriter, r *http.Request) {
...
func (m *Mux) methodNotAllowedResponse(w http.ResponseWriter, r *http.Request) {
